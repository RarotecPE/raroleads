...
import { createProposta } from "@/lib/proposal-actions";
...
export function ProposalRequestForm({ clientes, bases, modulos, contractedModuleIds }: {
  clientes: ClientOption[];
  bases: BaseOption[];
  modulos: ModuleOption[];
  contractedModuleIds: string[];
}) {
  const [tipo, setTipo] = useState<"implantacao_sistema" | "consultoria">("implantacao_sistema");
  const [origem, setOrigem] = useState<"cadastrado" | "avulso">("cadastrado");
  const [municipioId, setMunicipioId] = useState("");
  const [selectedBases, setSelectedBases] = useState<Set<string>>(new Set());
  const [selectedModules, setSelectedModules] = useState<Set<string>>(new Set());
  const [consultModuleId, setConsultModuleId] = useState("");
  const [draftBases, setDraftBases] = useState<DraftBase[]>([]);
  ...
  return <form action={createProposta} onSubmit={validate} className="flex flex-col gap-5">
    <section className="rounded-app-lg border border-app-border bg-app-surface p-4 sm:p-5">
      ...
      <div className="mt-3 grid gap-3 sm:grid-cols-2">{([ ["implantacao_sistema", "Implantação do sistema"], ["consultoria", "Consultoria"] ] as const).map(([value, label]) => <label key={value} className={`cursor-pointer rounded-app-md border p-4 ${tipo === value ? "border-app-primary bg-app-primary/10" : "border-app-border"}`}><input type="radio" name="tipo" value={value} checked={tipo === value} onChange={() => { setTipo(value); setOrigem("cadastrado"); resetSelections(""); }} className="mr-2 accent-app-primary" /><span className="font-semibold text-app-foreground">{label}</span></label>)}</div>
    </section>

    <section className="rounded-app-lg border border-app-border bg-app-surface p-4 sm:p-5">
      <h2 className="text-sm font-bold text-app-foreground">2. Cliente e escopo</h2>
      {tipo === "implantacao_sistema" ? <div className="mt-3 flex gap-2"><button type="button" className={origem === "cadastrado" ? btnPrimary : btnGhost} onClick={() => { setOrigem("cadastrado"); resetSelections(""); }}>Cliente cadastrado</button><button type="button" className={origem === "avulso" ? btnPrimary : btnGhost} onClick={() => { setOrigem("avulso"); resetSelections(""); setDraftBases([newDraftBase()]); }}>Município avulso</button></div> : null}

      {origem === "cadastrado" || tipo === "consultoria" ? <div className="mt-4 space-y-4">
        <Field label="Cliente"><select name="municipioId" required value={municipioId} onChange={(event) => resetSelections(event.target.value)} className={selectCls}><option value="" disabled>Selecione...</option>{clientes.map((cliente) => <option key={cliente.id} value={cliente.id}>{cliente.nome} · {cliente.municipio}/{cliente.uf}</option>)}</select></Field>
        {tipo === "consultoria" ? <>
          ...
          <Field label="Especificidades técnicas"><div className="grid gap-2 sm:grid-cols-2">{[["portal","Portal"],["sagres","Envio SAGRES"],["esocial","Envio eSocial"],["recadastramento","Recadastramento"]].map(([value,label]) => <label key={value} className="flex items-center gap-2 text-sm text-app-foreground"><input type="checkbox" name="especificidades" value={value} className="accent-app-primary" />{label}</label>)}</div></Field>
          <Field label="Atividades realizadas em conjunto com o município?"><div className="flex gap-5 text-sm text-app-foreground"><label><input required type="radio" name="atividadeConjunta" value="sim" className="mr-2 accent-app-primary" />Sim</label><label><input required type="radio" name="atividadeConjunta" value="nao" className="mr-2 accent-app-primary" />Não</label></div></Field>
        ...
        <div className="grid gap-4 sm:grid-cols-3"><Field label="Nome do cliente"><input name="clienteNomeSnapshot" required className={inputCls} placeholder="Ex.: Prefeitura Municipal" /></Field><Field label="UF"><select name="uf" value={uf} onChange={(event) => { setUf(event.target.value); setCodigoIbge(""); }} className={selectCls}>{UFS.map((value) => <option key={value}>{value}</option>)}</select></Field><Field label="Município"><select required value={codigoIbge} onChange={(event) => setCodigoIbge(event.target.value)} className={selectCls}><option value="" disabled>Selecione...</option>{ibgeCities.map((city) => <option key={city.id} value={city.id}>{city.nome}</option>)}</select></Field></div>
        <input type="hidden" name="codigoIbge" value={codigoIbge} /><input type="hidden" name="municipioNome" value={selectedCity?.nome ?? ""} />
    ...
    <section className="rounded-app-lg border border-app-border bg-app-surface p-4 sm:p-5"><Field label="Observações"><textarea name="observacoes" rows={4} className={textareaCls} /></Field></section>
    <div className="flex justify-end"><SubmitButton className={btnPrimary}>Solicitar proposta</SubmitButton></div>
  </form>;
}
