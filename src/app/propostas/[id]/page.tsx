import { ArrowLeft, Building2, Download, Eye, FileUp, Mail } from "lucide-react";
...
export default async function PropostaDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <div className="flex flex-col gap-5">
    ...
    {canManage && proposal.situacao === "gerada" ? <Panel><PanelHeader title="Enviar proposta" description="O documento mais recente será anexado ao e-mail" /><DialogForm action={enviarProposta} className="grid gap-4 p-4 sm:grid-cols-2"><input type="hidden" name="id" value={id} /><Field label="Nome do destinatário"><input name="destinatarioNome" required className={inputCls} /></Field><Field label="E-mail do destinatário"><input name="destinatarioEmail" type="email" required className={inputCls} /></Field><div className="flex justify-end sm:col-span-2"><SubmitButton className={btnPrimary}><Mail className="h-4 w-4" /> Enviar proposta</SubmitButton></div></DialogForm></Panel> : null}

    {canManage && proposal.situacao === "enviada" ? <Panel><PanelHeader title="Registrar retorno do cliente" /><div className="flex flex-wrap gap-2 p-4"><ProposalDecisionDialog id={id} status="aceita" canCreateRecords={Boolean(proposal.municipioId && hasPendingRecords)} /><ProposalDecisionDialog id={id} status="recusada" /><ProposalDecisionDialog id={id} status="em_retificacao" /></div></Panel> : null}

    {canManage && proposal.situacao === "aceita" && proposal.municipioId && hasPendingRecords ? <Panel><PanelHeader title="Cadastros pendentes" description="Ainda existem bases ou módulos da proposta sem vínculo no cadastro do cliente" /><DialogForm action={materializarCadastrosProposta} className="p-4"><input type="hidden" name="id" value={id} /><p className="text-sm text-app-muted-foreground">Crie agora os registros restantes. Bases sem CNPJ aparecerão nas pendências para que o cadastro seja finalizado na área de clientes.</p><div className="flex justify-end"><SubmitButton className={btnPrimary}><Building2 className="h-4 w-4" /> Criar bases e módulos</SubmitButton></div></DialogForm></Panel> : null}
