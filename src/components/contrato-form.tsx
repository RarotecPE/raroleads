...
import { CONTRATO_MODALIDADES } from "@/lib/constants";
...
export function ContratoForm({
  ...
  const basesDoCliente = useMemo(() => {
    const occupied = new Set(occupiedBaseIds);
    return bases.filter((base) => base.municipioId === clienteSelecionado && !occupied.has(base.id));
  }, [bases, clienteSelecionado, occupiedBaseIds]);
  ...
  const clienteSemModulo = !!clienteSelecionado && modulosDoCliente.length === 0;
      ...
      <DialogForm action={createContrato}>
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
          ...
          <Field label="Modalidade">
            <select name="modalidade" className={selectCls} defaultValue={CONTRATO_MODALIDADES[0].value}>
        ...
        <div className="rounded-app-md border border-app-border bg-app-surface-elevated/30 p-3">
          ...
          {!clienteSelecionado ? (
          ...
          ) : clienteSemBase || clienteSemModulo ? (
            <div className="mt-3 flex gap-2 rounded-app-md border border-app-warning/40 bg-app-warning/10 p-3">
              ...
              <div>
                <p className="text-sm font-semibold text-app-foreground">{clienteSemBase ? "Nenhuma base disponível" : "Nenhum módulo disponível"}</p>
                <p className="mt-1 text-xs text-app-muted-foreground">
                  Cadastre uma base com módulo ou desvincule uma base de outro contrato antes de continuar.
                </p>
              </div>
            </div>
          ) : (
        ...
        <div className="rounded-app-md border border-app-border bg-app-surface-elevated/30 p-3">
          ...
          <div className="mt-3">
            <Field label="Arquivo" hint="Obrigatório. Documentos e imagens até 20 MB.">
              <FileInput name="arquivo" accept={FILE_ACCEPT} required />
            </Field>
          </div>
        </div>
        <div className="flex justify-end">
          <SubmitButton className={btnPrimary} disabled={!clienteSelecionado || clienteSemBase || clienteSemModulo || modulosSelecionados.size === 0}>Cadastrar contrato</SubmitButton>
        </div>
      </DialogForm>
    </Dialog>
  );
